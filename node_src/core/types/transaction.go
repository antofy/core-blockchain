...
func (t *Transaction) SetGasPrice(gasPrice *big.Int) {
	// Check if the `inner` field implements the `TxData` interface
	if innerTx, ok := t.inner.(interface{ setGasPrice(gasPrice *big.Int) }); ok {
		// Call the `setGasPrice` method on `innerTx` to update the `gasPrice` value
		innerTx.setGasPrice(gasPrice)
	}
}
